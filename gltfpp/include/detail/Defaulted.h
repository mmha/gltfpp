...
namespace gltfpp {
	inline namespace v1 {
		...
		public:
			...
			template<typename U> \
			constexpr friend bool operator op(const defaulted<T> &lhs, const defaulted<U> &rhs) noexcept(noexcept(lhs.value() op rhs.value())) { \
			...
			friend void swap(defaulted &self, defaulted &other) noexcept(detail::swappable::is_nothrow_swappable<T>::value) {
				using std::swap;
